...
namespace Audio {
    ...
    void FetchALErrors(const std::filesystem::path& file, int line)
    {
        ...
        // MC_CORE_ASSERT_TAGGED(!error_found);
    }
    ...
    Engine::Engine()
    {
        MC_CORE_ASSERT_TAGGED(!s_Instance, "Audio Engine instance already created!");
        s_Instance = this;

        m_AlcDevice = alcOpenDevice(nullptr);
        CHECK_ALC_ERRORS(m_AlcDevice);
        MC_CORE_ASSERT_TAGGED(m_AlcDevice, "alcOpenDevice: Unable to create OpenAL device");

        m_AlcContext = alcCreateContext(m_AlcDevice, nullptr);
        CHECK_ALC_ERRORS(m_AlcDevice);
        MC_CORE_ASSERT_TAGGED(m_AlcContext, "alcCreateContext: Unable to create OpenAL context");

        if (!alcMakeContextCurrent(m_AlcContext))
            MC_CORE_ASSERT_TAGGED(false, "alcMakeCurrentContext: Could not set OpenAL context to current context");
        CHECK_ALC_ERRORS(m_AlcDevice);
    ...
    bool Engine::IsMusicPlaying(std::size_t index)
    {
        return s_Instance->m_MusicMixer.at(index).playing;
    }
    ...
    void Engine::EnginePollingThread()
    {
        while (!s_Instance->m_ShouldThreadClose) {
            ...
            MC_PROFILE_SCOPE_NAME("AudioEngine::Update");
            s_Instance->m_SfxMixerLock.lock();
    ...
    Engine::Sfx::Sfx(const std::string& sfxFileName)
    {
        ...
        if (!sndFile) {
            ...
            MC_CORE_ASSERT_TAGGED(false, "Could not open audio file");
        }

        // verify file has valid data
        if (sfinfo.frames < 1 || sfinfo.frames > (sf_count_t)(INT_MAX / sizeof(short)) / sfinfo.channels) {
            sf_close(sndFile);
            MC_CORE_ASSERT_TAGGED(false, "Audio file produced invalid data");
        }
        ...
        if (!format) {
            ...
            sf_close(sndFile);
            MC_CORE_ASSERT_TAGGED(false, "Unsuported channel count/format");
        }
        ...
        if (num_frames < 1) {
            free(membuf);
            sf_close(sndFile);
            MC_CORE_ASSERT_TAGGED(false, "Failed to read samples in audio file");
        }
