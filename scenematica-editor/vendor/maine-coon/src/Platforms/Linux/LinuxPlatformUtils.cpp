...
static void open_done(GObject* source, GAsyncResult* result, gpointer data)
{
    ...
    if (!file) {
        MC_CORE_ERROR("{0} {1} {2}", g_quark_to_string(error->domain), error->code, error->message);
        static_cast<GTKDialogContext*>(data)->error = true;
        g_error_free(error);
    } else {
        static_cast<GTKDialogContext*>(data)->path = g_file_peek_path(file);
        MC_CORE_INFO("File: {0}", static_cast<GTKDialogContext*>(data)->path);
        g_object_unref(file);
    }
}
